            ...
            if (文件名 == "config" || 文件名 == mytoken) {
                ...
                if (text === "null" && b64 === "null") {
                    ...
                    if (b64 === "null") {
                        ...
                        const value = await KV.get(文件名); // 验证写入
                        return new Response(value, {
                            status: 200,
                            headers: {
                                'content-type': 'text/plain; charset=utf-8',
                        ...
                        const decodedContent = base64Decode(空格替换加号(b64));
                        await KV.put(文件名, decodedContent);
                        const value = await KV.get(文件名); // 验证写入
                        return new Response(value, {
                            status: 200,
                            headers: {
...
TIMESTAMP=$(date +%s)
curl -k "https://$DOMAIN/$FILENAME?token=$TOKEN&b64=$BASE64_TEXT&t=$TIMESTAMP"
echo "更新数据完成"
`
}
    ...
    return `
      <html>
        ...
        <body>
          ...
          Windows脚本: <button type="button" onclick="window.open('https://${域名}/config/update.bat?token=${token}&t=' + Date.now(), '_blank')">点击下载</button>
          <br>
          <pre>使用方法: <code>&lt;update.bat&nbsp;ip.txt&gt;</code></pre>
          <br>
          Linux脚本: 
          <code>&lt;curl&nbsp;https://${域名}/config/update.sh?token=${token}&t=$(date +%s)&nbsp;-o&nbsp;update.sh&nbsp;&&&nbsp;chmod&nbsp;+x&nbsp;update.sh&gt;</code><br>
          <pre>使用方法: <code>&lt;./update.sh&nbsp;ip.txt&gt;</code></pre><br>
          <br>
          在线文档查询: <br>
          https://${域名}/<input type="text" name="keyword" placeholder="请输入要查询的文档">?token=${token}    
          <button type="button" onclick="window.open('https://${域名}/' + document.querySelector('input[name=keyword]').value + '?token=${token}&t=' + Date.now(), '_blank')">查看文档内容</button>
